...
"""hog.py is the only file you need to modify!!!"""
...
def roll_dice(num_rolls, dice=six_sided):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    score,curr_side,flag = 0,0,False
    while(num_rolls > 0):
        curr_side = dice()
        if(curr_side == 1):
            flag = True
...
def boar_brawl(player_score, opponent_score):
    ...
    ans = abs(ten_digit - one_digit)
    return 1 if ans == 0 else ans * 3
    # END PROBLEM 2
...
def always_roll(n):
    ...
    def always_roll_n(player_score, opponent_score):
        return n
    # Return a function have to arguments
...
def is_always_roll(strategy, goal=GOAL):
    ...
    # BEGIN PROBLEM 7
    "*** YOUR CODE HERE ***"
    prev_num_of_dice = strategy(0,0)
    # Use nested loop to validate every possible
    for player_score in range(goal):
        for opponent_score in range(goal):
            curr_num_of_dice = strategy(player_score,opponent_score)
            if(prev_num_of_dice != curr_num_of_dice):
                return False
            else:
                prev_num_of_dice = curr_num_of_dice
    return True
    # END PROBLEM 7
...
def make_averaged(original_function, samples_count=1000):
    ...
    def averaged_dice(*args):
        sum = 0
        if(len(args) == 2):
            num_rolls,dice = args[0],args[1]
            for i in range(samples_count):
                sum += original_function(num_rolls,dice)            
        else:
            for i in range(samples_count):
...
def max_scoring_num_rolls(dice=six_sided, samples_count=1000):
    ...
    # num_rolls from 1 to 10
    for num_rolls in range(1,11):
        curr_average_score = average_dice(num_rolls,dice)
        if(curr_average_score > max_average_score):
            max_average_score = curr_average_score
            max_num_rolls = num_rolls
        elif(curr_average_score == max_average_score):
            max_num_rolls = min(max_num_rolls,num_rolls)
