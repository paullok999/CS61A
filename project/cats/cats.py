    ...
    def match(paragraph):
        ...
        for i in range(len(subject)):
            for j in range(len(words)):
                if(lower(remove_punctuation(subject[i])) == lower(remove_punctuation(words[j]))):
                    return True
        return False
    return match
    # END PROBLEM 2
...
def wpm(typed, elapsed):
    ...
    # BEGIN PROBLEM 4
    "*** YOUR CODE HERE ***"
    # The formula for words per minute is the ratio of the number of characters (including spaces) 
    # typed divided by 5 (a typical word length) to the elapsed time in minutes.
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    # BEGIN PROBLEM 5
    "*** YOUR CODE HERE ***"
    smallest_diff,ret_word = sys.maxsize,""
    for i in range(len(word_list)):
        if(typed_word == word_list[i]):
            return word_list[i]
        curr_diff = diff_function(typed_word,word_list[i],limit)
        if(curr_diff < smallest_diff):
            smallest_diff = curr_diff
            ret_word = word_list[i]
    return typed_word if(smallest_diff > limit) else ret_word
    # END PROBLEM 5
...
def report_progress(typed, source, user_id, upload):
    ...
    # BEGIN PROBLEM 8
    "*** YOUR CODE HERE ***"
    hit,progress,n,m = 0,0,len(typed),len(source)
    for i in range(n):
        if(typed[i] != source[i]):
            break
        hit += 1
...
def time_per_word(words, timestamps_per_player):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    return match(words,[[timestamps_per_player[i][j] - timestamps_per_player[i][j - 1] for j in range(len(timestamps_per_player[i])) if(j > 0)] for i in range(len(timestamps_per_player))])
    # END PROBLEM 9
...
def fastest_words(match):
    ...
    ans = [[] for _ in range(len(times))]
    for j in range(len(times[0])):
        fast_player,min_time = 0,0x7FFFFFF
        for i in range(len(times)):
            if(times[i][j] < min_time):
                min_time = times[i][j]
                fast_player = i
        ans[fast_player].append(words[j])
    return ans
    # END PROBLEM 10
